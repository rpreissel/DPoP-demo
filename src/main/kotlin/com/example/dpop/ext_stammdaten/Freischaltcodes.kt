 ...
 * ([ausstellen], [widerrufen], the listings) is what the register's own mock UI (`/ext/`) and the
 * demo disclosure use.
 */
@Service
class Freischaltcodes(
    ...
    fun pruefe(personId: Long, codeHash: String): Boolean {
        val now = Instant.now()
    ...
    @Transactional
    fun ausstellen(personId: Long, gueltigBis: Instant): BriefView {
    ...
    @Transactional(readOnly = true)
    fun fuerPerson(personId: Long): List<FreischaltcodeView> {
        val now = Instant.now()
     ...
     */
    @Transactional(readOnly = true)
    fun juengsterGueltigerCode(personId: Long): String? {
        val now = Instant.now()
