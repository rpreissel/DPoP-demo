...
class AuthSmsLookupToolController(
    ...
    @PostMapping("/orchestrator/api/v1/app/channels/{channelSessionId}/tool-activate/auth-sms-lookup")
    ...
    fun activate(
        ...
        val context = controllerSupport.beginActivation(channelSessionId, bindingKeyRef, AUTH_SMS_LOOKUP_TOOL_ID, ToolCategory.AUTH)
        val outcome = handler.start(context.toolSession.toolSessionId!!)
        return ResponseEntity.status(HttpStatus.CREATED).body(controllerSupport.applyOutcome(AUTH_SMS_LOOKUP_TOOL_ID, outcome, context))
    }
