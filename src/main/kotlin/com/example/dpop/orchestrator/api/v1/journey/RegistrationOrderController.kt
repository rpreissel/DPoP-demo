...
import com.example.dpop.orchestrator.session.RegistrationOrderService
 ...
 * Demo scope deliberately: no auth guard, same as `ToolAvailabilityController` - do not expose
 * this beyond a trusted operator network as-is.
 */
@RestController
...
class RegistrationOrderController(private val registrationOrderService: RegistrationOrderService) {

    @GetMapping
    @Operation(summary = "Current REGISTER order")
    fun get(): RegistrationOrderState = RegistrationOrderState(registrationOrderService.isEnrollFirst())
    ...
    fun put(@RequestBody request: RegistrationOrderState) {
        registrationOrderService.setEnrollFirst(request.enrollFirst)
    }
}
