...
sealed interface KcSelectMethodState : JourneyState {
     ...
     * [com.example.dpop.orchestrator.domain.journey.JourneyContext] to ask directly. Declared on the
     * sealed interface itself (not just the one variant below) so `interpret` can read it without
     * a downcast, the same way [com.example.dpop.orchestrator.domain.journey.state.StepUpState.targetAcr] is.
     */
