 ...
 * see that function's doc for why an account's past `account.audit_event.acr` (IDENTIFIED) is deliberately NOT
...
class DefaultAuthPolicy(private val toolRegistry: ToolHandlerRegistry) : AuthPolicy {
     ...
     * persisted `account.audit_event.acr` (IDENTIFIED) from a past session: `AuthEvidence` is one-per-channel,
     ...
     * time"). `account.audit_event.acr` (IDENTIFIED)'s OWN role in the three-way cap (ADR-5) already happens
