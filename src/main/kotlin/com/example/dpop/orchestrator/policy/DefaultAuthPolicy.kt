 ...
 * see that function's doc for why an account's past `account.identification.achieved_acr` is deliberately NOT
...
class DefaultAuthPolicy(private val toolRegistry: ToolHandlerRegistry) : AuthPolicy {
     ...
     * persisted `account.identification.achieved_acr` from a past session: `AuthEvidence` is one-per-channel,
     ...
     * time"). `account.identification.achieved_acr`'s OWN role in the three-way cap (ADR-5) already happens
