 ...
 * both live. Replaces `EnrollEmailFlow`'s own local `EMAIL_PATTERN` (same regex, now shared) and
 * backs [AttributeRules.normalizeAnchorValue]'s `EMAIL` case, so `resolveByAnchor`/`anchorValue`/
 * `resolveAccountByEmail` and the `account_anchor` write path all agree on exactly one definition
