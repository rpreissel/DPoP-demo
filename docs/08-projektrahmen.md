...
- **M1a** `orchestrator.domain` (bis 2026-09-26 `orchestrator.kernel`) — Kein eigenes Modulith-Modul, sondern das unterste Paket **innerhalb** von `orchestrator`: die gemeinsamen Begriffe (`AuthIntent`, `AmrSource`, `AcrLevels`, `OrchestratorException`, `FeatureFlagProvider`, `ToolCatalog`), dazu der fachliche Kern – die Journey-Zustände, `IntentStrategy` mit `Transition`/`Action`, `AuthPolicy` und `AuthEvidence`. Es hängt von nichts anderem im Orchestrator ab und benutzt kein Framework (kein Spring, JPA, Jackson, Logging); beides prüft `OrchestratorArchitectureTest` ([Idee: Fachkern und Technik trennen](ideen/fachkern-und-technik-trennen.md)). Es gibt das Paket, weil fast jeder Paketzyklus im Orchestrator nur ein *Name* am falschen Ort war: `AmrSource` sagt zum Beispiel, woher ein Nachweis stammt (eine Frage der Richtlinie), lag aber neben der JPA-Entität, die den Nachweis speichert
