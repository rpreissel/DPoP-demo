# Archiv: Domänen-/DB-Modell-Review (`account`, `orchestrator`)

> **Archiv.** Dieser Review ist abgeschlossen. Das Dokument bleibt erhalten, weil es zeigt, wie
> die damaligen Befunde entschieden wurden, und wegen der Fallstricke am Ende, die weiterhin
> nützlich sind.
>
> Für den aktuellen Stand ist es nicht maßgeblich — das sind `docs/12-entscheidungen.md` und der
> Code. Zwei Verweise darin gehen ins Leere und bleiben trotzdem stehen:
> `docs/13-review-domaenen-db-modell.md` gibt es nicht mehr, der Inhalt steckt in den ADRs und im
> Code; die Migrationen `V30`, `V31` und `V33` sind inzwischen in `V1__schema.sql`
> zusammengefasst.

Dieses Dokument richtete sich ursprünglich an den nächsten Agenten, der diese Arbeit fortsetzt.
