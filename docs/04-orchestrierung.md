...
Die `-lookup`-Tools lösen den Account selbst über die eingegebene E-Mail auf (im jeweiligen Controller, nicht im Handler — Modulith-Grenze, [Projektrahmen](08-projektrahmen.md) A11) und melden ihn über `Completed.Authenticated.accountId` zurück ([Tool-Architektur](03-tool-architektur.md) Abschnitt 2); warum sie trotz gleicher `method` nicht mit ihrem geräte-gebundenen Zwilling kollidieren, steht dort ebenfalls (`ToolDescriptor.deviceBound`). Bei Erfolg schreibt der Orchestrator `DeviceAccountLink` neu — ein danach mit `intent="auto"` angelegter Kanal erkennt das Gerät und bietet den gewöhnlichen geräte-gebundenen LOGIN an, nicht wieder den Lookup-Login.
