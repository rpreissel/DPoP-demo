...
|---|---|---|---|---|
| REGISTRATION | `ident-*` | Completed | genau eine erlaubte Enrollment-Methode | `{type:"tool", toolId:"enroll-sms", step:"enroll"}` |
| REGISTRATION | `ident-*` | Completed | mehrere erlaubte Enrollment-Methoden | `{type:"flow", context:"enrollment", step:"selectMethod"}` plus `stepData={options:[...]}` |
