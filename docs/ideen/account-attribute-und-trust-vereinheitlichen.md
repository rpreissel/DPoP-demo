...
nicht mehr die Folge daraus. Der Vorschlag in der Tabelle (`createUnboundAccount`) ist offen; er ist präziser,
...
| `AuthMethodView` für gespeicherte Einträge mit Instanz-ID | `AccountMethodRegistrationView` | Diese Objekte beschreiben eine konkrete eingerichtete Methodeninstanz, nicht den Methodentyp. Wichtig bei mehreren Geräten. Nicht `Credential` nennen: das eigentliche Credential gehört weiterhin dem Methodenmodul und wird nur referenziert. API-Felder, IDs und EnrollmentRef-Semantik erhalten. |
