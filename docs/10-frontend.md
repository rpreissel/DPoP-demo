...
| FE-15 | Ein authentifizierter Kanal lässt sich gezielt auf ein höheres Sicherheitsniveau anheben (Step-up). | Der UI-Button „Auf loa2 anheben" ruft den Step-up-Auslöser (`POST /channels/{channelSessionId}/step-ups`, [API](05-api.md)) auf und erscheint nur, solange loa2 fehlt. Die API kann auch ein höheres Ziel anfordern; `ident-eid` ist dafür das vorhandene `loa3`-fähige Tool. Die Antwort läuft über dieselbe Tool-Navigation wie jeder andere Login/Step-up. |
