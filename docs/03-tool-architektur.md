...
- Von `allowsMultipleInstances` **getrennt**, obwohl `device` heute beides beantwortet. Die eine aus der anderen zu lesen hält nur, solange `device` die einzige Multi-Instanz-Methode ist: Eine künftige Methode, die lediglich mehrere parallele Instanzen erlaubt, ohne an einen Schlüssel gebunden zu sein, würde sonst still Geräte-Semantik erben, die sie nie beansprucht hat.
