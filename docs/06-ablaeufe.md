...
- **`enrolledUnderAcr` als eigenes Feld, nicht nur Audit-Inhalt**: Das effektive `achievedLoa` eines `auth-*`-Tools ist durch `enrolledUnderAcr` der verwendeten Methode gedeckelt ([Orchestrierung](04-orchestrierung.md) Abschnitt 1). Ohne diese Regel gäbe es einen Eskalationspfad: eine in schwacher Session hinterlegte Methode erzeugte dauerhaft ein höheres Niveau, als je nachgewiesen wurde. Den Wert kennt nur der Orchestrator, nie das Modul.
