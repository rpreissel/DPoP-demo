...
| `kobil_mock.*` (Fremdsystem) | — | **kein** Cleanup durch uns | `kobil_mock` simuliert KOBIL und untersteht nicht unserer Aufbewahrung. Dass es überhaupt persistiert, ist Voraussetzung und nicht Bequemlichkeit: ohne das zeigte nach jedem Neustart jede `auth_kobil.enrollment`-Zeile auf einen Nutzer, den es beim Anbieter nicht mehr gibt |
...
- **Eine Methode zu widerrufen hat bei KOBIL eine Außenhälfte.** `KobilEnrollmentCleanup` löscht nicht nur unsere Zeile, sondern räumt auch den Nutzer beim Anbieter ab — sonst bliebe dort ein gebundenes Gerät stehen, von dem hier nichts mehr weiß. Dieser zweite Aufruf ist ein nicht-transaktionaler Außeneffekt derselben Klasse wie der gemockte SMS-Versand: unsere Zeile verschwindet in jedem Fall.
