...
- Ein Ankerschreibvorgang hat einen **Preis**, und zwar zwei verschiedene: `AttributeType.anchorAcrFloor` deklariert je Attributtyp, welches Niveau das *Erstbinden* (`establish`) und welches das *Ersetzen* (`replace`) mindestens verlangt. `EMAIL` bindet bei `loa1` — eine Registrierung hat noch nichts bewiesen, ein loa2-Gate dort machte das erste Konto unmöglich —, ersetzt aber erst ab `loa2`: Das Ersetzen lenkt die Auflösung eines bestehenden Kontos um, und über den Anker finden drei Lookup-Verfahren ihr Konto. `PERSON_ID` verlangt schon zum Erstbinden `loa2`; sein `allowsAnchorReplacement = false` bleibt die führende Regel, der Floor ist nur die zweite Verteidigung. Geprüft wird an der einzigen Schreibstelle (`AccountService.recordAnchor`), Unterschreitung ist eine Abweisung (`409`), kein stilles Überspringen.
- Die Ankerzeile merkt sich das Niveau: `account.anchor.established_loa` ist das Gegenstück zu `account.auth_method.enrolled_under_acr` und trägt wie dieses das **tatsächlich bewiesene**, nach ADR-5 gedeckelte Niveau — nie das, was ein Tool für sich behauptet. Erst damit ist im Nachhinein beantwortbar, wie stark ein Anker belegt war.
