...
- `AccountRetraction` (`account.retraction`) macht einen Wert ungültig: eine Widerrufs-Zeile mit eigenem Vertrauensanker (`RetractionAnchor`: `ACCOUNT_MANAGEMENT`, `EXT_STAMMDATEN`, `OPERATOR`), Grund und Zeitpunkt ([12-entscheidungen.md](12-entscheidungen.md) ADR-12). „Aktuell gültig" ist Behauptungen minus Retraktionen — zeitbasiert: eine Retraktion cancelt nur Behauptungen, die vor ihr liegen; ein danach neu bezeugter Wert zählt wieder. Zwei Auslöser: Das Entfernen einer Methode zieht über `auth_method_id` deren Behauptungen zurück (nur die mit `AttributeAuthority.METHOD_MODULE`), und der In-place-Ersatz eines Ankers retrahiert den alten Wert (`ACCOUNT_MANAGEMENT`, Grund „anker-ersetzt"), damit das Log mit dem Anker übereinstimmt.
