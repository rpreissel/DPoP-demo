...
| A5 | Löschpfad unvollständig: `orchestrator.journey_log` und `orchestrator.attempt_throttle` blieben beim Kontolöschen stehen (DSGVO). | behoben, Teilannahme widerlegt | Die vermutete fehlende Cascade auf `account.claim`/`account.anchor` trifft **nicht** zu (`ON DELETE CASCADE` in `V30`/`V31`). `AccountDeletionService.deleteAccount` räumt `journey_log` über `account_id` **und** die Channel-Sessions in **einer** Anweisung ab (`flushAutomatically` + `clearAutomatically`), `attempt_throttle` nur in den kontobezogenen Scopes (`ACCOUNT`, `ACCOUNT_SEND`). `OrchestratorExceptionHandler` loggt `409 CONCURRENT_MODIFICATION` jetzt mit Entity und Id. |
...
| B1 | Full-Table-Scan im Identifikationspfad: drei nicht sargable `lower(trim(...))`-Abfragen, Schnittmenge in der Anwendung, ohne Obergrenze — trivialer DoS vor jeder Authentisierung. | behoben | Migration `V34__account_attribute_normalized_value.sql` (Spalte `normalized_value`, Index `idx_account_attribute_type_normalized`); `AccountClaim` (damals noch `AccountAttribute`) füllt sie über `@PrePersist`/`@PreUpdate` plus `normalize()`. Eine Abfrage `findAccountIdsMatchingAllThree` ersetzt die drei; `CANDIDATE_LIMIT` 50 + 1, Überschreitung ergibt `Resolution.Ambiguous`, nie einen Treffer. |
